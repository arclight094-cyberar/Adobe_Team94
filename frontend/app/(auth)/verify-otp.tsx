...
export default function VerifyOtp() {
  ...
  const handleVerify = async () => {
    ...
    // Validate OTP
    if (otpCode.length !== 4) {
      showAlert('error', 'ERROR', 'Please enter the complete 4-digit OTP');
      return;
    }

    if (!userEmail) {
      showAlert('error', 'ERROR', 'Email not found. Please sign up again.');
      router.back();
      return;
    }
    ...
    try {
      ...
      if (response.ok && data.success === true) {
        ...
        showAlertFromResponse(200, 'OTP verified successfully! Please login to continue.');
        setTimeout(() => {
          router.replace('/(auth)/login');
        }, 1500);
      } else {
        // Handle error response from backend
        showAlert('error', 'ERROR', data.message || 'OTP verification failed. Please try again.');
        // Clear OTP on error
        setOtp(['', '', '', '']);
